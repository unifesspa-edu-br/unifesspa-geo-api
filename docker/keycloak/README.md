...
A imagem é **a mesma** em todos os ambientes; o que muda é o backend de banco:

- **Testes de integração (CI e local)** sobem o Keycloak em `start-dev` com **H2 efêmero** (a fixture não define `KC_DB`), por simplicidade de provisionamento. Não há migração de schema persistida — trocar a tag da imagem é transparente para a suíte.
- **`docker-compose.yml` (dev local)** usa **Postgres** com volume persistente (`postgres_data`).
